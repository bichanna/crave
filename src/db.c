...
bool db_init_schema(sqlite3 *db) {
  static const char *schema =
      ...
      "FOR EACH ROW "
      "BEGIN "
      "  UPDATE recipes SET updated_at = strftime('%s','now') WHERE id = "
      "OLD.id;"
      "END;";
...
static long upsert_tag(sqlite3 *db, const char *name) {
  ...
  if (sqlite3_prepare_v2(db, "INSERT OR IGNORE INTO tags(name) VALUES(?1);", -1,
                         &st, NULL) != SQLITE_OK) {
  ...
  st = NULL;
  if (sqlite3_prepare_v2(db, "SELECT id FROM tags WHERE name = ?1;", -1, &st,
                         NULL) != SQLITE_OK) {
...
int db_load_summaries(sqlite3 *db, RecipeSummary *out, int max_out) {
  ...
  if (sqlite3_prepare_v2(
          ...
          "WHERE rt.recipe_id = ?1 ORDER BY t.name COLLATE NOCASE;",
          -1, &tg, NULL) != SQLITE_OK) {
  ...
  while (n < max_out && sqlite3_step(st) == SQLITE_ROW) {
    ...
    while (s->tag_count < CRAVE_MAX_TAGS && sqlite3_step(tg) == SQLITE_ROW) {
      copy_col(s->tags[s->tag_count], CRAVE_TAG_CAP,
               sqlite3_column_text(tg, 0));
      s->tag_count++;
    }
    n++;
  }
...
static bool load_children(sqlite3 *db, Recipe *r) {
  ...
  // ingredients
  if (sqlite3_prepare_v2(db,
                         "SELECT amount, name FROM ingredients WHERE "
                         "recipe_id=?1 ORDER BY position, id;",
                         -1, &st, NULL) != SQLITE_OK) {
    db_log(db, "prepare ingredients");
    return false;
  }
  sqlite3_bind_int64(st, 1, r->id);
  while (r->ingredient_count < CRAVE_MAX_INGREDIENTS &&
  ...
  // equipment
  if (sqlite3_prepare_v2(db,
                         "SELECT name FROM equipment WHERE recipe_id=?1 ORDER "
                         "BY position, id;",
                         -1, &st, NULL) != SQLITE_OK) {
  ...
  sqlite3_bind_int64(st, 1, r->id);
  while (r->equipment_count < CRAVE_MAX_EQUIPMENT &&
         sqlite3_step(st) == SQLITE_ROW) {
  ...
  if (sqlite3_prepare_v2(
          db,
          "SELECT text FROM steps WHERE recipe_id=?1 ORDER BY position, id;",
          -2, &st, NULL) != SQLITE_OK) {
  ...
  while (r->tag_count < CRAVE_MAX_TAGS && sqlite3_step(st) == SQLITE_ROW) {
    copy_col(r->tags[r->tag_count++], CRAVE_TAG_CAP,
...
bool db_load_recipe(sqlite3 *db, long id, Recipe *out) {
  ...
  sqlite3_stmt *st = NULL;
  if (sqlite3_prepare_v2(db,
                         "SELECT id, title, description, image_path, servings, "
                         "prep_mins, cook_mins FROM recipes WHERE id=?1;",
                         -1, &st, NULL) != SQLITE_OK) {
...
static bool insert_children(sqlite3 *db, const Recipe *r, long rid) {
  ...
  // ingredients
  if (sqlite3_prepare_v2(db,
                         "INSERT INTO ingredients(recipe_id, position, amount, "
                         "name) VALUES(?1,?2,?3,?4);",
                         -1, &st, NULL) != SQLITE_OK) {
  ...
  for (int i = 0; i < r->ingredient_count; i++) {
    if (r->ingredients[i].amount[0] == '\0' &&
        r->ingredients[i].name[0] == '\0')
      continue;
    sqlite3_reset(st);
  ...
  if (sqlite3_prepare_v2(
          db,
          "INSERT INTO equipment(recipe_id, position, name) VALUES(?1,?2,?3);",
          -1, &st, NULL) != SQLITE_OK) {
  ...
  // steps
  if (sqlite3_prepare_v2(
          db, "INSERT INTO steps(recipe_id, position, text) VALUES(?1,?2,?3);",
          -1, &st, NULL) != SQLITE_OK) {
...
static bool delete_children(sqlite3 *db, long rid) {
  sqlite3_stmt *st = NULL;
  static const char *tables[] = {"ingredients", "equipment", "steps",
                                 "recipe_tags"};
...
long db_save_recipe(sqlite3 *db, const Recipe *r) {
  ...
  if (rid <= 0) {
    if (sqlite3_prepare_v2(
            db,
            "INSERT INTO recipes(title, description, image_path, servings, "
            "prep_mins, cook_mins) VALUES(?1,?2,?3,?4,?5,?6);",
            -1, &st, NULL) != SQLITE_OK) {
    ...
    if (sqlite3_prepare_v2(
            db,
            "UPDATE recipes SET title=?1, description=?2, image_path=?3, "
            "servings=?4, prep_mins=?5, cook_mins=?6, "
            "updated_at=strftime('%s','now') WHERE id=?7;",
            -1, &st, NULL) != SQLITE_OK) {
...
bool db_delete_recipe(sqlite3 *db, long id) {
  sqlite3_stmt *st = NULL;
  if (sqlite3_prepare_v2(db, "DELETE FROM recipes WHERE id=?1;", -1, &st,
                         NULL) != SQLITE_OK) {
...
int db_all_tags(sqlite3 *db, char out[][CRAVE_TAG_CAP], int max_out) {
  ...
  if (sqlite3_prepare_v2(db,
                         "SELECT DISTINCT t.name FROM tags t JOIN recipe_tags "
                         "rt ON rt.tag_id=t.id ORDER BY t.name COLLATE NOCASE;",
                         -1, &st, NULL) != SQLITE_OK) {
...
bool db_image_path_in_use(sqlite3 *db, const char *rel) {
  ...
  if (sqlite3_prepare_v2(db,
                         "SELECT 1 FROM recipes WHERE image_path=?1 LIMIT 1;",
                         -1, &st, NULL) != SQLITE_OK)
    return false;
